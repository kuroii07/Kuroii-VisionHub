...
- 0.3.9 批量队列基础能力已进入前端：AI 创作台可把当前文生图 / 图生图参数加入本地批量队列，侧边栏和首页可进入“批量队列”页面查看任务快照、配置实例、模型、尺寸、数量和参考图持久化提示；当前已开放“单任务确认执行 / 取消 / 失败任务重新入队 / 失败或已取消任务删除 / 连续队列执行与停止”路径，删除只移除本地队列任务，不删除作品画廊记录或磁盘图片。
...
- `0.3.9` 批量队列与多模型对比 V1
  - 进行中：已新增本地批量队列数据层、任务快照执行器骨架、AI 创作台“加入队列”按钮、“批量队列”页面，以及单任务执行前确认、执行状态回写、取消标记、失败任务重新入队、失败 / 取消任务删除和连续队列执行 V1。
  - 待继续：多 Prompt 队列、多尺寸队列和失败任务重试。
  - 待继续：同一 Prompt 多模型 / 多配置档案横向对比。
