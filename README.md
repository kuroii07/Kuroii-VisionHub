...
- 本地作品画廊
  ...
  - 删除图册记录，但不删除磁盘图片文件
  - 作品画廊 V2 计划已沉淀到 `docs/library-v2-plan.md`
- 免费平台
  - 管理常用免费 AI 图片平台入口
...
本仓库当前处在“作品画廊 V2 开发前”的检查点：

- 平台接入面板已经完成实例化配置重构。
- `/v1/responses` 图片结果解析已兼容常见 `result` / `image` 字段。
- Responses 长任务已加入 background + store 尝试和轮询查询；中转站不支持时会回退同步请求。
- 设置面板、页面标题、图标 Tooltip、免费平台 Logo、作品画廊指标压缩和图生图参考图细节已做过一轮优化。
- 作品画廊 V2 会从现有画廊升级为图片管理工具，方案见 [docs/library-v2-plan.md](docs/library-v2-plan.md)。
...
- 作品画廊 V2 的隐藏详情侧栏、底部悬浮搜索、视图模式、收藏、过滤、排序和图片管理能力已写入计划文档。
...
- 灵感中心高级能力：网站预设、批量导入、反推提示词、相似风格聚类
- 参考图来源详情查看
- 图册批量管理与导出
