...
- App version is now `0.5.23`, synchronized across package metadata, Tauri metadata, Cargo metadata, Cargo lock, app version display, README, and roadmap docs.
- Moved Provider config text cleanup, runtime Provider profile shape checking, and immutable current-model option completion from `src/ui/App.tsx` into `src/services/providerConfigValidation.ts`.
- The helper behavior remains unchanged: only strings are trimmed, profile-like values require string `id` and `providerId`, and a missing non-empty current model is prepended without mutating the original config.
- `App.tsx` continues to own clipboard reads, config JSON parsing, state updates, profile persistence, Provider config saves, credentials, model refresh/probe, diagnostics, and generation.
- Added 18 focused Vitest cases covering string/non-string cleanup, valid/invalid profile shapes, empty/existing/missing model handling, referential identity, ordering, and source-object immutability.
- `run_checks.ps1` passed with 128/128 frontend tests and 2/2 Rust tests; `npm.cmd audit --audit-level=high` reported 0 vulnerabilities.
- The `0.5.23` user-facing release EXE passed a 12-second launch smoke: file/product version `0.5.23`, 17,462,784 bytes (16.65 MB), SHA256 `0754F17C9E6AB6E0F6E35A19406406CE827E3B2CB9A849C5884A138DD775A86F`.
...
- App version is now `0.5.21`, synchronized across package metadata, Tauri metadata, Cargo metadata, Cargo lock, app version display, README, and roadmap docs.
- Moved profile-to-template ownership, legacy profiles without `serviceTemplateId` fallback matching, the six profile filter states, filter option/count generation, and filter matching from `src/ui/App.tsx` into `src/services/providerProfileSelection.ts`.
- The legacy ownership fallback remains unchanged for the established aggregator, OpenAI, MiniMax, Gemini, and local SD WebUI templates; profile ids and `profile:${profileId}` credential bindings are untouched.
- `App.tsx` continues to own profile creation, deletion, enable/disable, selected-profile fallback, credential deletion, configuration updates, connection tests, diagnostics, generation, and persistence.
- Added 18 focused Vitest cases covering provider mismatch, explicit template ids, every legacy fallback, filter order/counts/translations, enabled filtering, and each test-status filter.
- `run_checks.ps1` passed with 102/102 frontend tests and 2/2 Rust tests; `npm.cmd audit --audit-level=high` reported 0 vulnerabilities.
- The `0.5.21` user-facing release EXE passed a 12-second launch smoke: file/product version `0.5.21`, 17,463,296 bytes (16.65 MB), SHA256 `ACEBCF1A77FD1B484121BA87EBD978BD69E7CE3C876813FB67A0D07572E2A12F`.
...
- App version is now `0.5.15`, synchronized across package metadata, Tauri metadata, Cargo metadata, Cargo lock, app version display, README, and roadmap docs.
- Moved the existing lazy-mounted `CachedInspirationPage` wrapper and its active-page preview composition from `src/ui/App.tsx` into `src/ui/CachedInspirationPage.tsx`.
- Inspiration loading state, preview state, navigation callbacks, reference reuse, Prompt reuse/template creation, image reverse settings, settings navigation, import refresh state, and persistence remain owned by `App.tsx` or the existing Inspiration Center services.
- Smoke, UI QA, and release-candidate checks now track the extracted module and guard the exact App mount mappings, Inspiration Center prop forwarding, active-only preview behavior, preview navigation, one-way dependency boundary, and reduced App size.
- `run_checks.ps1` passed with 34/34 Provider tests and 2/2 Rust tests; `npm.cmd audit --audit-level=high` reported 0 vulnerabilities.
- The `0.5.15` user-facing release EXE passed a 12-second launch smoke: version `0.5.15`, 17,462,272 bytes (16.65 MB), SHA256 `092D4CEECF8C01E8D7F6386B86443CD37932436071E67DB02C86436247ABA128`.

### v0.5.14 app dialogs module extraction

- App version is now `0.5.14`, synchronized across package metadata, Tauri metadata, Cargo metadata, Cargo lock, app version display, README, and roadmap docs.
- Moved `BatchQueueNameDialog`, `ConfirmDialog`, `UtilityModalShell`, `ShortcutsModal`, `SystemInfoModal`, their local UI state types, and shortcut presentation definitions from `src/ui/App.tsx` into `src/ui/AppDialogs.tsx`.
- Queue storage and naming actions, confirmation requests and error state, global shortcut handling, storage settings, ComfyUI workflow management, and all mutating callbacks remain owned by `App.tsx`.
- `SystemInfoModal` now receives the App-owned version through a read-only `appVersion` prop; the extracted module does not import `App.tsx`.
- Smoke and UI QA checks now follow the extracted module and guard the App mounts, critical prop mappings, accessibility names, one-way dependency boundary, and reduced App size.
- `run_checks.ps1` passed with 34/34 Provider tests and 2/2 Rust tests; `npm.cmd audit --audit-level=high` reported 0 vulnerabilities.
- The `0.5.14` user-facing release EXE passed a 12-second launch smoke: version `0.5.14`, 17,462,784 bytes (16.65 MB), SHA256 `1603D5D883A70A164D345A494EA622C24D546368BD5ABBB58FA8262ADEFA12EE`.
...
- App version is now `0.5.12`, synchronized across package metadata, Tauri metadata, Cargo metadata, Cargo lock, app version display, README, and roadmap docs.
- Moved the existing `BatchQueuePage` implementation from the oversized `src/ui/App.tsx` into `src/ui/BatchQueuePage.tsx`.
- Queue selection, creation, rename/delete safety, serialized execution, pause/resume, task retry/cancel/delete, batch templates, comparison results, empty states, and preview behavior remain unchanged.
- `App.tsx` continues to own queue storage, task execution, confirmation dialogs, result writeback, active queue state, and every mutating callback.
- Smoke and UI QA checks now follow the extracted module and guard the real App mount, all critical callback mappings, empty states, accessibility names, and the one-way module boundary.
- `npm.cmd run verify` passed with 34/34 Provider tests and 2/2 Rust tests; `npm.cmd audit` reported 0 vulnerabilities.
- The `0.5.12` user-facing release EXE passed a 12-second launch smoke: 17,462,784 bytes (16.65 MB), SHA256 `E3A0357DE90FFF945200C2DD19F4DFD5DB152D95B6617C9B51F255B8D388807F`.
