...
- 0.3.9 批量队列基础能力已进入前端：AI 创作台可把当前文生图 / 图生图参数加入本地批量队列，侧边栏和首页可进入“批量队列”页面查看任务快照、配置实例、模型、尺寸、数量和参考图持久化提示；当前已开放“单任务确认执行 / 取消 / 失败任务重新入队”路径，连续执行仍暂时禁用，避免未确认前批量消耗额度。
...
- `0.3.9` 批量队列与多模型对比 V1
  - 进行中：已新增本地批量队列数据层、任务快照执行器骨架、AI 创作台“加入队列”按钮、“批量队列”页面，以及单任务执行前确认、执行状态回写、取消标记和失败任务重新入队。
