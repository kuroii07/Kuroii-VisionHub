...
- 0.3.10 仍作为 0.3 系列收口补丁，不新增大功能；后续主线进入 `0.4.0` 项目资产库 V1。
...
- `0.3.x` 只做收口补丁，`0.4.x` 进入项目资产库、Prompt 工作流、Provider 稳定、本地模型、数据治理和全局 QA；发布准备、release notes、安装包、迁移说明和 SHA256 记录整体后移到 `0.5.0` / `v1.0` 前。
...
- `0.4.0` 项目资产库 V1
  - 围绕单个创作项目管理参考图、Prompt、生成图、风格说明、批量队列和输出记录。
  - 从作品画廊和灵感中心建立项目关联，不复制磁盘图片，不删除原始记录。
  - AI 创作台支持选择当前项目，新生成记录自动带项目关系。
- `0.4.1` Prompt 与灵感工作流 V3
  - 提示词网站摘录、Prompt 组合器、灵感图片反推记录字段和跨页面复用。
  - 从当前 Prompt 保存为模板、摘录或项目 Prompt。
...
- `0.4.4` 数据治理与迁移 V1
  ...
  - 换电脑迁移向导：Provider profile id、系统凭据、图库目录、灵感目录、项目资产库和提示词润色凭据。
