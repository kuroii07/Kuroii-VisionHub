...
本仓库当前处在“0.3.9 批量队列与多模型对比 V1 进行中”阶段：
...
- 工作区首页 V2 已完成收口：启动页进入真实工作台首页，集中展示最近继续、异常任务、最近素材、项目资产库入口、批量队列入口、多模型对比入口和关键状态概览。
...
- 0.3.9 批量队列与多模型对比 V1 已进入前端：AI 创作台可把当前文生图 / 图生图参数加入本地批量队列，也可在“批量变体”按多 Prompt × 多尺寸创建任务，或在“多模型对比”选择多个配置实例，一键按同一 Prompt 创建对比组任务；侧边栏和首页可进入“批量队列”页面查看任务快照、对比组、配置实例、模型、尺寸、数量和参考图持久化提示。当前已开放“单任务确认执行 / 取消 / 失败任务重新入队 / 失败或已取消任务删除 / 连续队列执行与停止”路径，删除只移除本地队列任务，不删除作品画廊记录或磁盘图片。
...
- AI 创作台新增“批量变体”：支持逐行输入多个 Prompt，并勾选当前比例下的多个输出尺寸，一键创建多 Prompt × 多尺寸组合任务；单次最多创建 40 个任务，仍需到批量队列页确认执行。
- 批量队列页新增对比组统计和任务标识，可看到同组任务的序号；真实执行仍需进入队列确认，不会因为创建对比组而立即消耗额度。
- 失败 / 已取消任务删除、失败重新入队和连续执行能力继续保留；作品画廊记录不会因删除队列快照而被删除。
...
- 新增作品保存偏好：文件命名规则、按日期分组和按项目分组；该偏好不会移动已有图库文件，后续批量导出和项目资产库会优先读取。
...
- 项目资产库、批量队列、多模型对比以明确入口呈现；未完整落地的批量队列和多模型对比保持规划状态，不提供误导性的执行按钮。
...
- `0.3.9` 批量队列与多模型对比 V1
  ...
  - 已完成：多 Prompt / 多尺寸批量变体 V1，可按逐行 Prompt 和多个尺寸组合创建队列任务，单次限制 40 个任务，避免误操作创建过大队列。
  - 已完成：同一 Prompt 多配置实例横向对比 V1，可在创作台勾选多个配置实例并创建对比组入队，队列页显示对比组统计和任务标识。
  - 待继续：对比结果并排查看、更细的连续执行进度、暂停恢复体验和批量任务模板保存。
