...
- `0.3.2` 免费平台 V2
  - 免费平台分类：免费额度、需要登录、国内可用、海外平台、文生图、图生图、商用不确定。
  - 平台字段补齐：官网、账号要求、中文支持、图生图支持、免费额度、水印 / 分辨率限制、商用备注。
  - 使用状态：未使用、已注册、常用、暂不可用。
  - 快速操作：打开平台、复制推荐 Prompt、保存备注、标记常用。
  - 与提示词库联动，为不同免费平台准备平台专用 Prompt 写法。
- `0.3.3` 提示词库 V2
  - 模板分类：商业海报、电商主图、角色设定、游戏资产、图标 UI、社媒封面、图生图改写、免费平台专用。
  - 模板变量：`{主体}`、`{风格}`、`{场景}`、`{颜色}`、`{镜头}`、`{平台}`。
  - 模板编辑器：新增、编辑、删除、收藏、最近使用、导入、导出。
  - Prompt 组合：主体模板 + 风格模板 + 光影模板 + 负面约束。
