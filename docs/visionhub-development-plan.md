...
### 5.37 `0.5.10` free generation module extraction

Status 2026-07-10: completed and release-launch validated. The existing web platform helper has been moved out of the oversized app shell without changing its visible UI, platform list, local preference keys, logo cache, callbacks, or imported-result workflow.

Objectives:

- Move `FreeGenerationPage` into `src/ui/FreeGenerationPage.tsx` by preserving the existing component implementation.
- Keep prompt adaptation, clipboard/open behavior, imported-result persistence, and inspiration refresh orchestration in `App.tsx`.
- Preserve platform filters, card/list modes, favorites, status, notes, logo fallback caching, detail drawer, prompt copy/open, and web-result import behavior.
- Add smoke guards requiring the extracted module, its actual JSX mount, the App callback bridge, and no reverse imports into `App.tsx`.

Acceptance:

- [x] The extracted component matches the backed-up implementation after newline normalization.
- [x] `src/ui/App.tsx` is reduced below 9,000 lines and no longer defines `FreeGenerationPage`.
- [x] The free generation module does not import `App.tsx`.
- [x] Frontend production build, smoke checks, and UI QA pass after extraction.
- [x] Unified verification passed with 34/34 Provider tests and 2/2 Rust tests; `npm.cmd audit` reported 0 vulnerabilities.
...
### 5.41 `0.5.14` app dialogs module extraction

Status 2026-07-11: completed and release-launch validated. Shared app dialogs and their local presentation state have been moved out of the oversized app shell without changing visible UI, CSS classes, queue naming behavior, confirmation behavior, keyboard shortcut content, system information content, or ComfyUI workflow management.

Objectives:

- Move `BatchQueueNameDialog`, `ConfirmDialog`, `UtilityModalShell`, `ShortcutsModal`, and `SystemInfoModal` into `src/ui/AppDialogs.tsx`.
- Keep queue storage/actions, confirmation request state, global shortcut handling, storage settings, ComfyUI workflow management, and all mutating callbacks owned by `App.tsx`.
- Move shortcut presentation definitions and dialog-local state types with the extracted components while keeping `APP_VERSION` App-owned.
- Extend smoke, UI QA, and release-candidate tracking checks to cover the extracted module and exact App prop mappings.

Acceptance:

- [x] The extracted dialog components and shortcut presentation definitions mechanically match the backed-up implementation, except for the declared export and `appVersion` prop transformation.
- [x] `src/ui/App.tsx` is reduced below 6,650 lines and no longer defines the five extracted dialog components.
- [x] `src/ui/AppDialogs.tsx` does not import `App.tsx`; App retains state, callbacks, workflow management, and persistence responsibilities.
- [x] Frontend production build, smoke checks, and UI QA pass after extraction.
- [x] Unified verification passed with 34/34 Provider tests and 2/2 Rust tests; `npm.cmd audit --audit-level=high` reported 0 vulnerabilities.
...
### 5.43 `0.5.16` ComfyUI workflow presentation module extraction

Status 2026-07-13: completed and release-launch validated. ComfyUI workflow presentation types, status labels, the shared summary panel, and the workflow manager modal have been moved out of the oversized app shell without changing visible UI, CSS classes, workflow selection/deletion behavior, Provider behavior, generation behavior, or persistence.

Objectives:

- Move workflow presentation types, `workflowFormatLabel`, `comfyUIWorkflowRunStatus`, `ComfyUIWorkflowSummaryPanel`, and `ComfyUIWorkflowManagerModal` into `src/ui/ComfyUIWorkflowPresentation.tsx`.
- Keep workflow JSON parsing, import/clear actions, active preset selection, delete state updates, local storage, Provider diagnostics, and ComfyUI generation owned by `App.tsx`.
- Preserve the Provider settings summary mount and the manager modal's existing select/delete callbacks and shared `UtilityModalShell` composition.
- Extend smoke, UI QA, and release-candidate tracking checks to cover the extracted module, exact App callback mappings, and one-way dependency boundary.

Acceptance:

- [x] The extracted types, helpers, and components mechanically match the backed-up implementation except for declared exports and imports.
- [x] `src/ui/App.tsx` is reduced below 6,450 lines and no longer defines ComfyUI workflow presentation types, labels, summary panel, or manager modal.
- [x] `src/ui/ComfyUIWorkflowPresentation.tsx` does not import `App.tsx`; App retains parsing, mutations, Provider behavior, generation, and persistence responsibilities.
- [x] Smoke checks and UI QA pass after extraction.
- [x] Unified verification passed with 34/34 Provider tests and 2/2 Rust tests; `npm.cmd audit --audit-level=high` reported 0 vulnerabilities.
...
- Preserve the complete `main` history and the existing reference-pack branch in the new repository.
- Update only repository-facing links and version metadata; keep AppData, Tauri identifier, Cargo package name, internal executable name, keyring service, Provider profile ids, and credentials unchanged.
- Build and launch-check the Windows portable EXE, MSI, and NSIS artifacts from the final `0.5.25` source.
- Publish the first release in the new repository as an unsigned Windows pre-release with hashes and migration notes.

Acceptance:

- [x] New `origin` points to `kuroii07/Kuroii-VisionHub`; the former repository is retained as `old-origin` for rollback.
- [x] `main` history and `reference-packs/company-generated-images-2026-06-12` exist in the new repository.
